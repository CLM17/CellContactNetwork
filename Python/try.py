...
labels, dist = distance_transform_watershed(th_dapi, th_dapi, connectivity=np.ones((3,3)), sigma=1, min_distance=10)

io.imshow(labels,cmap='Greys')
output_path_labels = os.path.join(well_folder, 'dist_trans_wts.tif')
output_path_markers = os.path.join(well_folder, 'dist_trans_wts_dist.tif')
io.imsave(output_path_labels, labels)
io.imsave(output_path_markers, dist)
