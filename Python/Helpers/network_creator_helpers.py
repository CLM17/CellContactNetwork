...
#%%
def normalize_images_in_list(img_list):
    '''
    This function normalizes all images in the list img_list.
    '''
    
    normalized_img_list = []
    
    for img in img_list:
        normalized_img_list.append(img / np.max(img))
...
#%%
def find_nuclei_com(th_dapi):
    '''
    Function finds the center of mass (COM) of nuclei.
    Input: img = a grayscale image of DAPI channel
    Output: com = array with centers of mass, lbl = labels of objects.
    '''

    # Label all nuclei with a seperate integer
    lbl, n_obj = ndimage.label(th_dapi)

    # find center of mass of labels
    com_list = ndimage.measurements.center_of_mass(th_dapi, lbl, np.arange(1,n_obj+1))
...
#%%
def distance_transform_watershed(th_dapi, th_marker, connectivity=np.ones((3,3)), sigma=1, min_distance=10):
    '''
    ...
    '''
    dist = ndimage.distance_transform_edt(th_dapi)
    ...
    labels = watershed(dist_smooth, markers=markers, connectivity=connectivity, mask=th_marker, watershed_line=True)
...
def create_HELA_network(folder, well):
    ...
    print('Reading image '+well+'...')
    well_folder = os.path.join(folder, well)
    full_img = io.imread( os.path.join( well_folder, well+'_fused.tif') )
    
    th_dapi = io.imread( os.path.join( well_folder, well+'_th_dapi.tif') )
    th_marker = io.imread( os.path.join (well_folder, well+'_th_marker.tif') )

    ch_dapi = full_img[:,:,2]
    ch_marker = full_img[:,:,0]
    
    [th_dapi, th_marker, ch_dapi, ch_marker] = normalize_images_in_list([th_dapi, th_marker, ch_dapi, ch_marker])
    ...
    if th_method_MEM == 'unet':
        ...
        ch_marker = ch_marker[ymin:ymax, xmin:xmax]
    print('cropped')
    '''
    ...
    print('Removing the outside of the well...')
    xc_well, yc_well, diameter_well = get_well_locations(folder, well)
    ch_dapi, ch_marker = remove_outside_well([ch_dapi, ch_marker], xmin, ymin, xc_well, yc_well, diameter_well)

    print('Finding the nuclei centers of mass...')
    com, lbl_dapi = find_nuclei_com(th_dapi)
    
    print('Creating markers for the seeded watershed...')
    markers = make_markers(com, N, M)
    
    print('Seeded watershed...')
    wts = seeded_watershed(th_marker, markers)

    print('Doing connected component labeling to find the network...')
    contact_matrix = find_network(wts)

    print('Converting network matrix to a graph structure...')
    ...
    output_img = draw_network(G, ch_dapi, ch_marker)
