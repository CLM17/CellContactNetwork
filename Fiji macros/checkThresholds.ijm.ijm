...
run("Tiles to Fused", "tileDirectory="+tileFolder+" wellLocationsTextFile="+textFileConfigurations+" uiService= "+" opService= ");
getDimensions(width, height, channels, slices, frames);
rename("threshold");
run("Split Channels");

title3 = "Make adjustments?";
message3 = "If you want to, make adjustments. Use color picker and brush tool.\n Press OK when you are done.";
...
for (i = 0; i < channels; i++) {
	selectWindow("C"+d2s(i+1,0)+"-threshold");
	chName = getString("Name of this channel:", "dapi");
	waitForUser(title3, message3);

	selectWindow("C"+d2s(i+1,0)+"-threshold");
	save(wellFolder + "/" + well+"_th_"+ chName +".tif");)
}